...
public class MqttClientWrapper 
{
    ...
    public MqttClientWrapper(ILogger logger, IHaContext ha, INetDaemonScheduler scheduler, IKitchenLightsWrapper kitchenLightsWrapper, IFrontRoomLightsWrapper frontRoomLightsWrapper)
    {
        _logger = logger;
        _ha = ha;
        ...
        _doorLocksController = new DoorLocksController(_logger, ha, scheduler, kitchenLightsWrapper, frontRoomLightsWrapper);
        ...
        _cameraImageTaker = new CameraImageTaker(_logger);
    ...
    private async Task StartMqttListener()
    {
        ...
        _frontDoorLock = _entities.Lock.FrontDoorDeadbolt;
...
// Tag UID is: {asciiPayload}");
    ...
    private async Task checkIfDenTouchscreenButtons(string topic, string asciiPayload)
    {
        ...
        {
            if (_entities.Light.DenLightsGroup.IsOn())
            {
                _entities.Light.DenLightsGroup.TurnOff();
            }
            else
            {
                _entities.Light.DenLightsGroup.TurnOn();   
            }
        }
        
        if (asciiPayload.ToLower() == "tv_power_button_pressed")
        {
            // if (_entities.MediaPlayer.Tv.IsOn())
            // {
