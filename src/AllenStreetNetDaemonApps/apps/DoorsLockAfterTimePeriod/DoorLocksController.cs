...
namespace AllenStreetNetDaemonApps.DoorsLockAfterTimePeriod;
...
public class DoorLocksController
{
    ...
    public DoorLocksController(ILogger logger, IHaContext ha, INetDaemonScheduler scheduler, IKitchenLightsWrapper kitchenLightsWrapper, IFrontRoomLightsWrapper frontRoomLightsWrapper)
    {
        _logger = logger;
        ...
        FrontDoorLock = new AutomaticallyLockableLock(_logger, TimeSpan.FromMinutes(3), _entities.Lock.FrontDoorDeadbolt);
        BackDoorLock = new AutomaticallyLockableLock(_logger, TimeSpan.FromMinutes(6), _entities.Lock.BackDoorDeadbolt);
    ...
    private async Task WorkLockDisableTimes()
    {
        ...
        var frontLockDisableValue = 0m;
        var backLockDisableValue = 0m;
        ...
        try
        {
            ...
            frontLockDisableValue = decimal.Parse(frontLockDisableText);
            backLockDisableValue = decimal.Parse(backLockDisableText);
        }
        catch (FormatException e)
        {
            ...
            _logger.Information(e, "Failed to parse lock disable textboxes");
