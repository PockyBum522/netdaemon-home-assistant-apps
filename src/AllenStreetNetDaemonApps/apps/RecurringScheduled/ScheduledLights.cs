...
namespace AllenStreetNetDaemonApps.RecurringScheduled;
...
public class ScheduledLights
{
    ...
    public ScheduledLights(ILogger logger, IHaContext ha, INetDaemonScheduler scheduler)
    {
        _logger = logger;
        _ha = ha;
    ...
    {
        _entities.Light.FrontPorchLightsGroup.TurnOn();
        
        _entities.Switch.BackPorchChristmasLights.TurnOn();
        
        _entities.Light.DenLampLightsGroup.TurnOn();

        _entities.Light.KitchenUnderCabinetLightsGroup.TurnOn();
    ...
    private void SetLightsAtMorning()
    {
        ...
        _entities.Light.FrontPorchLightsGroup.TurnOff();
