...
//     {
//         _logger.LogInformation("Checking bulbLastOnAt: {LastOnAt} vs DateTimeOffset.Now - TimeSpan.FromMinutes(5): {FiveMinutesAgo}", _bulbLastOnAt, DateTimeOffset.Now - TimeSpan.FromMinutes(5));
//         if (_bulbLastOnAt >  DateTimeOffset.Now - TimeSpan.FromMinutes(5)) return;
//         
//         _logger.LogInformation("bulbLastOnAt was more than five minutes ago, so Checking espresso watts: {CurrentWatts}", espressoWatts);
//         if (espressoWatts < 50) return;
//
//         var isEspressoBulbOn = _kitchenLightsWrapper.IsEspressoBulbOn();
//         _logger.LogInformation("Was over threshold, checking if espresso bulb is on: {IsBulbOn}", isEspressoBulbOn);
//         if (isEspressoBulbOn) return;
//
//         _logger.LogInformation("Bulb was not on, setting espresso scene now");
//         
