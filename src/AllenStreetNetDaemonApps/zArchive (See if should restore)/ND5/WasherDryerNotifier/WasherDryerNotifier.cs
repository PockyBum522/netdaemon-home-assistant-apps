...
//
// namespace AllenStreetNetDaemonApps.WasherDryerNotifier;
//
// [NetDaemonApp]
...
//     }
//     
//     public WasherDryerNotifier(ILogger logger, IHaContext ha, INetDaemonScheduler scheduler)
//     {
//         _logger = logger;
...
//         
//         // _logger.LogDebug("Time until off: {TimeUntilOff}, Minutes only: {TimeUntilMinutes} | countdownMessage: {CountdownMessage}", 
...
//         {
//             //_logger.LogInformation("_entities.InputText.LaundryCountdown.State {WasherString}", _entities.InputText.LaundryCountdown.State);
//             
...
//     {
//         var washerCurrentWattsRaw = _entities.Sensor.WasherDryerCurrentMonitoringPlugElectricConsumptionW.State;
//         
...
//         }
//
//         _logger.LogTrace("Washer power usage history count: {HistoryCount}", _washerPowerUsageHistory.Count);
//
//         _logger.LogTrace("Washer power usage history: {@WasherPowerUsageHistory}", _washerPowerUsageHistory);
//
//         var averagedValue = _washerPowerUsageHistory.AsQueryable().Average();
//         
//         _logger.LogDebug("Washer power usage history AVERAGE: {Average} out of {Count} values", averagedValue.ToString("F2"), _washerPowerUsageHistory.Count);
//     }
//
...
//             case WasherState.Washing:
//                 
//                 _logger.LogDebug("Washer has been running for: {ElapsedTime}", DateTimeOffset.Now - _washerStartedAt);
//                 
...
//                 {
//                     _logger.LogInformation("Washer started less than 4 minutes ago, preventing transition to other states until then");
//                     break;
//                 }
//                 
//                 // Triggers:
...
//             case WasherState.StayFresh:
//                 
//                 _logger.LogDebug("LOAD IS FINISHED, PLEASE UNLOAD WASHER!");
//                 
//                 // Triggers:
//                 
...
//             case WasherState.Problem:
//                 
//                 _logger.LogDebug("WASHER HAS A PROBLEM HALLLLLLLLP");
//
//                 // Triggers:
//                 
...
//                 {
//                     _logger.LogWarning("Washer was in WasherState.Problem, but load started more than 24h ago so timing out and moving to WasherState.Off");
//                  
//                     changeStateTo(WasherState.Off);
//                 }
//                 
...
//         
//         _logger.LogDebug("STATE: {OldStateName} => {NewStateName}", oldStateName, newStateName);
//
